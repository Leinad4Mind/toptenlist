<?php
namespace micogian\toptenlist\event;
	...
	public function setup($event)	
	{
		...
		*/
		...
		function resize_thumbs($physical,$subfolder,$attach_id, $root_path)
		{
			...
			$source = imagecreatefromjpeg($root_path . "files/" . $physical);
		...
		*/
		...
		$forum_list1	= "2,9,10,11,15";
		$forum_list2	= "2,9,10,11,15";
		$forum_list3	= "2,9,10,11,15";
		$forum_list4	= "2,9,10,11,15";
		...
		if ($mode_cor == 1 || $mode_cor == '')
		{
			...
			*/
			//$where_list = 'pf.parent_id IN(3,4,5)';   // Elenco dei parent_id da elaborare
			//$where_list = 'pt.forum_id IN(6,7,9,10)';  // Elenco dei forum_id da elaborare   
			...
			WHERE pt.forum_id IN (".$forum_list1.")
		...
		}elseif ($mode_cor == 2)
		{
			...
			$sql2 = "SELECT tt.topic_id, tt.forum_id, tt.topic_title, tt.topic_time, tt.topic_moved_id, tt.topic_first_poster_name,
				...
				WHERE tt.forum_id IN (".$forum_list2.")
			...
			while ($row2 = $this->db->sql_fetchrow($result2))
			{
				if ($this->auth->acl_get('f_read', $row2['forum_id']) == 1) 
				{
					if ($n2 < 10)
					{
						...
						// $last_topic_data[$n2]   		= $this->user->format_date($row2['topic_time'], "|d M|");
		...
		}elseif ($mode_cor == 3)
		{
			...
			$sql3 = "SELECT tt.topic_id, tt.forum_id, tt.topic_moved_id, tt.topic_last_post_id, tt.topic_last_poster_id, tt.topic_last_poster_name, tt.topic_last_post_subject, tt.topic_last_post_time,
				ft.forum_id, ft.forum_name
				FROM " . TOPICS_TABLE . " tt, " . FORUMS_TABLE . " ft 
				WHERE tt.forum_id IN (". $forum_list3 .")
				AND tt.topic_type = 0
				AND tt.topic_moved_id = 0
				AND tt.forum_id = ft.forum_id
				AND tt.topic_visibility=1
...
*/
			...
			ft.forum_id, ft.forum_name 
			FROM " . TOPICS_TABLE . " tt, " . FORUMS_TABLE . " ft
			WHERE tt.forum_id IN(".$forum_list4.")
			AND tt.forum_id = ft.forum_id
